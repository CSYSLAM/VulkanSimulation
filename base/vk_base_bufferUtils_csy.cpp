...
void BufferUtilsCsy::CreateBuffer(DeviceCsy* device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    // Create buffer
...
void BufferUtilsCsy::CopyBuffer(DeviceCsy* device, VkCommandPool commandPool, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void BufferUtilsCsy::CreateBufferFromData(DeviceCsy* device, VkCommandPool commandPool, void* bufferData, VkDeviceSize bufferSize, VkBufferUsageFlags bufferUsage, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    BufferUtilsCsy::CreateBuffer(device, bufferSize, stagingUsage, stagingProperties, stagingBuffer, stagingBufferMemory);
    ...
    BufferUtilsCsy::CreateBuffer(device, bufferSize, usage, flags, buffer, bufferMemory);

    // Copy data from staging to buffer
    BufferUtilsCsy::CopyBuffer(device, commandPool, stagingBuffer, buffer, bufferSize);
