...
InstanceCsy::InstanceCsy(const char* applicationName, unsigned int additionalExtensionCount, const char** additionalExtensions) {
    ...
    //std::vector<const char*> extensions(additionalExtensionCount);
    //std::memcpy(extensions.data(), additionalExtensions, sizeof(char*) * additionalExtensionCount);
...
VkInstance InstanceCsy::GetVkInstance() {
    return instance;
}


VkPhysicalDevice InstanceCsy::GetPhysicalDevice() {
    return physicalDevice;
}


const VkSurfaceCapabilitiesKHR& InstanceCsy::GetSurfaceCapabilities() const {
    return surfaceCapabilities;
}


const QueueFamilyIndices& InstanceCsy::GetQueueFamilyIndices() const {
    return queueFamilyIndices;
}


const std::vector<VkSurfaceFormatKHR>& InstanceCsy::GetSurfaceFormats() const {
    return surfaceFormats;
}


const std::vector<VkPresentModeKHR>& InstanceCsy::GetPresentModes() const {
    return presentModes;
}


uint32_t InstanceCsy::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const {
...
VkFormat InstanceCsy::GetSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const {
...
//void InstanceCsy::initDebugReport() {
//    if (ENABLE_VALIDATION) {
...
void InstanceCsy::PickPhysicalDevice(std::vector<const char*> deviceExtensions, QueueFlagBits requiredQueues, VkSurfaceKHR surface) {
...
DeviceCsy* InstanceCsy::CreateDevice(QueueFlagBits requiredQueues, VkPhysicalDeviceFeatures deviceFeatures) {
    ...
    return new DeviceCsy(this, vkDevice, queues);
}


InstanceCsy::~InstanceCsy() {
    if (ENABLE_VALIDATION) {
