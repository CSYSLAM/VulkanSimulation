...
SwapChainCsy::SwapChainCsy(DeviceCsy* device, VkSurfaceKHR vkSurface, unsigned int numBuffers)
...
void SwapChainCsy::Create() {
    ...
    // Create swap chain
    VK_CHECK_RESULT(vkCreateSwapchainKHR(device->GetVkDevice(), &createInfo, nullptr, &vkSwapChain));
...
void SwapChainCsy::Destroy() {
    vkDestroySwapchainKHR(device->GetVkDevice(), vkSwapChain, nullptr);
}

VkSwapchainKHR SwapChainCsy::GetVkSwapChain() const {
    return vkSwapChain;
}

VkFormat SwapChainCsy::GetVkImageFormat() const {
    return vkSwapChainImageFormat;
}

VkSurfaceFormatKHR SwapChainCsy::GetSurfaceFormat() const {
    return surfaceFormat;
}

VkExtent2D SwapChainCsy::GetVkExtent() const {
    return vkSwapChainExtent;
}

uint32_t SwapChainCsy::GetIndex() const {
    return imageIndex;
}

uint32_t SwapChainCsy::GetCount() const {
    return static_cast<uint32_t>(vkSwapChainImages.size());
}

VkImage SwapChainCsy::GetVkImage(uint32_t index) const {
...
VkSemaphore SwapChainCsy::GetImageAvailableVkSemaphore() const {
    return imageAvailableSemaphore;
}

VkSemaphore SwapChainCsy::GetRenderFinishedVkSemaphore() const {
    return renderFinishedSemaphore;
}

void SwapChainCsy::Recreate() {
    Destroy();
    Create();
}

bool SwapChainCsy::Acquire() {
