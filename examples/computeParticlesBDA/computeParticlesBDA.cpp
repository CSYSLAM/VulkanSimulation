...
public:
	...
	VkDeviceAddress storageBufferDeviceAddress = 0;
	VkDeviceAddress uniformBufferDeviceAddress = 0;

	PFN_vkGetBufferDeviceAddressKHR vkGetBufferDeviceAddressKHR{ VK_NULL_HANDLE };
    VkPhysicalDeviceBufferDeviceAddressFeatures enabledBufferDeviceAddresFeatures{};
    
    // 用于传递粒子缓冲区地址的推送常量
    struct ParticleBufferReference {
        VkDeviceAddress particlesReference;
        VkDeviceAddress uniformReference;
    };
	...
	void buildComputeCommandBuffer()
	{
		...
		// 使用推送常量传递缓冲区地址
		ParticleBufferReference references{};
		references.particlesReference = storageBufferDeviceAddress;
		references.uniformReference = uniformBufferDeviceAddress;
		vkCmdPushConstants(compute.commandBuffer, compute.pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(ParticleBufferReference), &references);
    ...
    bufferDeviceAddressInfo.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
		bufferDeviceAddressInfo.buffer = storageBuffer.buffer;
		storageBufferDeviceAddress = vkGetBufferDeviceAddressKHR(device, &bufferDeviceAddressInfo);
	}
	...
	void prepareGraphics()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "computeParticlesBDA/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "computeParticlesBDA/particle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		pushConstantRange.size = sizeof(ParticleBufferReference);
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "computeparticlesBDA/particle.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
	...
	void prepareUniformBuffers()
	{
		...
		VkBufferDeviceAddressInfo bufferDeviceAddressInfo{};
		bufferDeviceAddressInfo.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
		bufferDeviceAddressInfo.buffer = compute.uniformBuffer.buffer;
		uniformBufferDeviceAddress = vkGetBufferDeviceAddressKHR(device, &bufferDeviceAddressInfo);
