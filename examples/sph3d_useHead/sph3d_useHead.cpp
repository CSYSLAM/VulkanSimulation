...
public:
	...
	VulkanExample() : VulkanExampleBase()
	{
		...
		camera.setRotation(glm::vec3(-30.0f, -15.0f, 0.0f));
	...
	void prepareStorageBuffers()
	{
		...
		int resX = (int)((volumeEnd.x - volumeBegin.x) / dx);
		int resY = (int)((volumeEnd.y - volumeBegin.y) / dx);
		int resZ = (int)((volumeEnd.z - volumeBegin.z) / dx);
		...
		VkDeviceSize indexBufferSize = PARTICLE_COUNT * sizeof(uint32_t);
	...
	void prepareGraphics()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "sph3d_useHead/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "sph3d_useHead/particle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			...
			vks::initializers::descriptorSetLayoutBinding(
				...
				VK_SHADER_STAGE_COMPUTE_BIT,
				2),
			// Binding 3 : cellIndex buffer
			vks::initializers::descriptorSetLayoutBinding(
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				VK_SHADER_STAGE_COMPUTE_BIT,
				3),
			// Binding 4 : cellOffset buffer
			vks::initializers::descriptorSetLayoutBinding(
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				VK_SHADER_STAGE_COMPUTE_BIT,
				4),
		...
		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(
				...
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				3,
				&cellIndexBuffer.descriptor),
			// Binding 4 : cellOffsetBuffer buffer
			vks::initializers::writeDescriptorSet(
				compute.descriptorSet,
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				4,
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "sph3d_useHead/particle.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
