...
public:
	...
	struct Compute {
		...
		struct UniformData {						// Compute shader uniform block object
			...
			uint32_t bufferIndex = 0;  // 添加缓冲区索引
		} uniformData;
	} compute;
	...
	{
		title = "Compute shader particle system (Bindless)";
	...
	void prepareGraphics()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "computeParticlesBDLESS/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "computeParticlesBDLESS/particle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			// Binding 0: 粒子位置存储缓冲区数组 (bindless)
			vks::initializers::descriptorSetLayoutBinding(
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				VK_SHADER_STAGE_COMPUTE_BIT,
				0,
				1), // 使用数组大小为1，但将使用更新后绑定标志
		...
		// 为存储缓冲区设置更新后绑定标志
		std::vector<VkDescriptorBindingFlags> bindingFlagsArray = {
			VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT_EXT | 
			VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT_EXT,
			0  // UBO 不需要特殊标志
		};

		bindingFlags.bindingCount = static_cast<uint32_t>(bindingFlagsArray.size());
		bindingFlags.pBindingFlags = bindingFlagsArray.data();
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "computeParticlesBDLESS/particle.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
