...
namespace CsyVk {
	...
	std::set<CsyVk::ObjectId>& DirectedAcyclicGraph::vertices()
	{
		return mVertices;
	}

	std::map<CsyVk::ObjectId, std::unordered_set<CsyVk::ObjectId>>& DirectedAcyclicGraph::edges()
	{
		return mEdges;
	}

	std::map<CsyVk::ObjectId, std::unordered_set<CsyVk::ObjectId>>& DirectedAcyclicGraph::reverseEdges()
	{
