...
int main(int argc, char* argv[])
{
	...
	CsyVk::VkContext* ctx = new CsyVk::VkContext(physicalDevice);
	...
	std::shared_ptr<csyvk::Buffer> bufferA = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	std::shared_ptr<csyvk::Buffer> bufferB = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	std::shared_ptr<csyvk::Buffer> bufferC = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	ctx->createBuffer(
	...
	std::shared_ptr<csyvk::Buffer> stageA = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	std::shared_ptr<csyvk::Buffer> stageB = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	std::shared_ptr<csyvk::Buffer> stageC = std::make_shared<csyvk::Buffer>(ctx->deviceHandle());
	ctx->createBuffer(
	...
	setLayoutBindings.push_back(csyvk::initializers::descriptorSetLayoutBinding(VkVariable::descriptorType(VariableType::DeviceBuffer), VK_SHADER_STAGE_COMPUTE_BIT, 0));
	setLayoutBindings.push_back(csyvk::initializers::descriptorSetLayoutBinding(VkVariable::descriptorType(VariableType::DeviceBuffer), VK_SHADER_STAGE_COMPUTE_BIT, 1));
	setLayoutBindings.push_back(csyvk::initializers::descriptorSetLayoutBinding(VkVariable::descriptorType(VariableType::DeviceBuffer), VK_SHADER_STAGE_COMPUTE_BIT, 2));

	VkDescriptorSetLayoutCreateInfo descriptorLayout =
		csyvk::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);

	VkDescriptorSetLayout descriptorSetLayout = VK_NULL_HANDLE;
	vkCreateDescriptorSetLayout(ctx->deviceHandle(), &descriptorLayout, nullptr, &descriptorSetLayout);

	std::vector<VkDescriptorPoolSize> poolSizes;
	poolSizes.push_back(csyvk::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3));

	VkDescriptorPool descriptorPool = VK_NULL_HANDLE;

	// Create the descriptor pool
	VkDescriptorPoolCreateInfo descriptorPoolInfo =
		csyvk::initializers::descriptorPoolCreateInfo(poolSizes, poolSizes.size());
	vkCreateDescriptorPool(ctx->deviceHandle(), &descriptorPoolInfo, nullptr, &descriptorPool);

	VkDescriptorSetAllocateInfo allocInfo =
		csyvk::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
	...
	pushConstantRanges.push_back(csyvk::initializers::pushConstantRange(VK_SHADER_STAGE_COMPUTE_BIT, sizeof(uint), 0));

	VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo =
		csyvk::initializers::pipelineLayoutCreateInfo(&descriptorSetLayout, 1);
	...
	VkCommandBufferAllocateInfo cmdBufAllocateInfo =
		csyvk::initializers::commandBufferAllocateInfo(mCommandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
	...
	writeDescriptorSets.push_back(
		csyvk::initializers::writeDescriptorSet(descriptorSet, VkVariable::descriptorType(VariableType::DeviceBuffer), 0, &bufferA->descriptor));
	writeDescriptorSets.push_back(
		csyvk::initializers::writeDescriptorSet(descriptorSet, VkVariable::descriptorType(VariableType::DeviceBuffer), 1, &bufferB->descriptor));
	writeDescriptorSets.push_back(
		csyvk::initializers::writeDescriptorSet(descriptorSet, VkVariable::descriptorType(VariableType::DeviceBuffer), 2, &bufferC->descriptor));
