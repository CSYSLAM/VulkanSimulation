...
#include "vk_native_device_array3D.h"
#include "vk_native_uniform.h"
#include "vk_native_constant.h"
#include <memory>
#include <map>
...
#define CONSTANT(T) constantPtr<T>()
	...
	VkProgram::VkProgram(Args... args)
	{
		...
		VkCommandBufferAllocateInfo cmdBufAllocateInfo =
			CsyVkN::initializers::commandBufferAllocateInfo(mCommandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
		...
		VkSemaphoreCreateInfo semaphoreCreateInfo = CsyVkN::initializers::semaphoreCreateInfo();
		...
		for (size_t i = 0; i < mFormalParamters.size(); i++)
		{
			...
			{
				setLayoutBindings.push_back(CsyVkN::initializers::descriptorSetLayoutBinding(VkVariable::descriptorType(mFormalParamters[i]->type()), VK_SHADER_STAGE_COMPUTE_BIT, i));
			}
		}

		VkDescriptorSetLayoutCreateInfo descriptorLayout =
			CsyVkN::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
		...
		if (nBuffer > 0)
			poolSizes.push_back(CsyVkN::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, nBuffer));

		if (nUniform > 0)
			poolSizes.push_back(CsyVkN::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, nUniform));

		// Create the descriptor pool
		VkDescriptorPoolCreateInfo descriptorPoolInfo =
			CsyVkN::initializers::descriptorPoolCreateInfo(poolSizes, poolSizes.size());
		VK_CHECK_RESULT(vkCreateDescriptorPool(ctx->deviceHandle(), &descriptorPoolInfo, nullptr, &descriptorPool));

		VkDescriptorSetAllocateInfo allocInfo =
			CsyVkN::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
		...
		VkFenceCreateInfo fenceInfo = CsyVkN::initializers::fenceCreateInfo(VK_FLAGS_NONE);
		...
		for (size_t i = 0; i < variables.size(); i++)
		{
			...
			if ((mAllArgs[i]->type() == VariableType::DeviceBuffer && mAllArgs[i]->bufferSize() > 0) || mAllArgs[i]->type() == VariableType::Uniform) {
				writeDescriptorSets.push_back(
					CsyVkN::initializers::writeDescriptorSet(descriptorSet, VkVariable::descriptorType(variable->type()), i, &variable->getDescriptor()));
			}
		}
