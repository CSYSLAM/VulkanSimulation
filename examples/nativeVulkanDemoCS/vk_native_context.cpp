...
*/
	...
	*/
	VkResult VkContext::createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, VkDeviceSize size, VkBuffer *buffer, VkDeviceMemory *memory, void *data)
	{
		...
		VkBufferCreateInfo bufferCreateInfo = CsyVkN::initializers::bufferCreateInfo(usageFlags, size);
		bufferCreateInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
		VK_CHECK_RESULT(vkCreateBuffer(logicalDevice, &bufferCreateInfo, nullptr, buffer));

		// Create the memory backing up the buffer handle
		VkMemoryRequirements memReqs;
		VkMemoryAllocateInfo memAlloc = CsyVkN::initializers::memoryAllocateInfo();
		...
		if (data != nullptr)
		{
			...
			{
				VkMappedMemoryRange mappedRange = CsyVkN::initializers::mappedMemoryRange();
	...
	* @return VK_SUCCESS if buffer handle and memory have been created and (optionally passed) data has been copied
	*/
	VkResult VkContext::createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, std::shared_ptr<CsyVkN::Buffer> &buffer, VkDeviceSize size, const void *data)
	{
    ...
    * @return VK_SUCCESS if buffer handle and memory have been created and (optionally passed) data has been copied
    */
    VkResult VkContext::createBuffer(uint32_t poolType, std::shared_ptr<CsyVkN::Buffer> &buffer, const void *data)
    {
	...
	*/
	void VkContext::copyBuffer(CsyVkN::Buffer *src, CsyVkN::Buffer *dst, VkQueue queue, VkBufferCopy *copyRegion)
	{
	...
	*/
	...
	{
		VkCommandBufferAllocateInfo cmdBufAllocateInfo = CsyVkN::initializers::commandBufferAllocateInfo(pool, level, 1);
		VkCommandBuffer cmdBuffer;
		VK_CHECK_RESULT(vkAllocateCommandBuffers(logicalDevice, &cmdBufAllocateInfo, &cmdBuffer));
		// If requested, also start recording for the new command buffer
		if (begin)
		{
			VkCommandBufferBeginInfo cmdBufInfo = CsyVkN::initializers::commandBufferBeginInfo();
	...
	*/
	void VkContext::flushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue, VkCommandPool pool, bool free)
	{
		...
		VkSubmitInfo submitInfo = CsyVkN::initializers::submitInfo();
