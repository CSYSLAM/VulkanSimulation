...
namespace CsyVkN {
	...
	public:
		...
		VkResult        createBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, std::shared_ptr<CsyVkN::Buffer> &buffer, VkDeviceSize size, const void *data = nullptr);
        VkResult        createBuffer(uint32_t poolType, std::shared_ptr<CsyVkN::Buffer> &buffer, const void *data = nullptr);
        VkResult        createMemoryPool(VkInstance instance, uint32_t apiVerion);
		void            copyBuffer(CsyVkN::Buffer *src, CsyVkN::Buffer *dst, VkQueue queue, VkBufferCopy *copyRegion = nullptr);
