...
namespace CsyVkN
{
	...
	void VkProgram::update(bool sync)
	{
		...
		VkSubmitInfo computeSubmitInfo = CsyVkN::initializers::submitInfo();
	...
	void VkProgram::addGraphicsToComputeBarriers(VkCommandBuffer commandBuffer)
	{
		if (ctx->isComputeQueueSpecial()) {
			VkBufferMemoryBarrier bufferBarrier = CsyVkN::initializers::bufferMemoryBarrier();
	...
	bool VkProgram::load(std::string fileName)
	{
		...
		{
			pushConstantRanges.push_back(CsyVkN::initializers::pushConstantRange(VK_SHADER_STAGE_COMPUTE_BIT, mFormalConstants[i]->bufferSize(), i));
		}

		VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo =
			CsyVkN::initializers::pipelineLayoutCreateInfo(&descriptorSetLayout, 1);
		...
		// Create pipeline
		VkComputePipelineCreateInfo computePipelineCreateInfo = CsyVkN::initializers::computePipelineCreateInfo(pipelineLayout, 0);
	...
	VkPipelineShaderStageCreateInfo VkProgram::createComputeStage(std::string fileName)
	{
		...
		shaderStage.module = CsyVkN::tools::loadShaderModule(fileName, ctx->deviceHandle());
	...
	VkMultiProgram::VkMultiProgram()
	{
		...
		VkCommandBufferAllocateInfo cmdBufAllocateInfo =
			CsyVkN::initializers::commandBufferAllocateInfo(commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1);
