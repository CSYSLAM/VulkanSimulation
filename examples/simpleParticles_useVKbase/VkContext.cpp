...
namespace CsyVk {
	...
	VkResult VkContext::setLogicalDevice(VkDevice inputDevice,
		std::vector<VkQueue> inputQueue, std::vector<uint32_t> inputQueueFamilyIndex, bool useUniQueue)
	{
		logicalDevice = inputDevice;

		queueFamilyIndices.graphics = inputQueueFamilyIndex[0];
		queueFamilyIndices.compute = inputQueueFamilyIndex[1];
		queueFamilyIndices.transfer = inputQueueFamilyIndex[2];

		graphicsQueue = inputQueue[0];
		computeQueue = inputQueue[1];
		transferQueue = inputQueue[2];
