...
std::string shaderDir = "glsl/simpleParticles_useVKbase/";
...
public:
	...
	VkBuffer particlesBuffer_ = VK_NULL_HANDLE;
	...
	void Render()
	{
		...
		if (device->GetVkDevice() == VK_NULL_HANDLE)
		{
			std::cerr << "[ERROR] VkDevice is null!" << std::endl;
			abort();
		}
		if (swapchain_ == VK_NULL_HANDLE)
		{
			std::cerr << "[ERROR] swapchain_ is null!" << std::endl;
			abort();
		}
		if (imageAvailableSemaphore_ == VK_NULL_HANDLE)
		{
			std::cerr << "[ERROR] imageAvailableSemaphore_ is null!" << std::endl;
			abort();
		}

		VkResult res = vkAcquireNextImageKHR(device->GetVkDevice(), swapchain_, UINT64_MAX, imageAvailableSemaphore_, VK_NULL_HANDLE, &imageIndex_);
		if (res != VK_SUCCESS)
		{
			std::cerr << "[ERROR] vkAcquireNextImageKHR failed with error: " << res << std::endl;
			abort();
		}
		
		if (imageIndex_ >= graphicsCommandBuffer_.size())
		{
			std::cerr << "[ERROR] imageIndex_=" << imageIndex_ << " is out of bounds (size=" << graphicsCommandBuffer_.size() << ")!" << std::endl;
			abort();
		}
	...
	VulkanExample()
	{
		...
		//NEW
		vkData.instance_ = instance->GetVkInstance();
		vkData.physicalDevice_ = instance->GetPhysicalDevice();
		vkData.device_ = device ->GetVkDevice();
		vkData.queue_.push_back(device->GetQueue(QueueFlags::Graphics));
		vkData.queue_.push_back(device->GetQueue(QueueFlags::Compute));
		vkData.queue_.push_back(device->GetQueue(QueueFlags::Transfer));
		vkData.queueFamilyIndex_.push_back(device->GetQueueIndex(QueueFlags::Graphics));
		vkData.queueFamilyIndex_.push_back(device->GetQueueIndex(QueueFlags::Compute));
		vkData.queueFamilyIndex_.push_back(device->GetQueueIndex(QueueFlags::Transfer));
		
		CsyVk::VkSystem::instance()->initializeWithInstance(vkData);
		///
		CsyVk::DArray<Vertex> dA(NUM_PARTICLES);
		...
		CsyVk::CArray<Vertex> initialParticlePosition(NUM_PARTICLES);
		for (int i = 0; i < NUM_PARTICLES; ++i) {
			float f = static_cast<float>(i) / (NUM_PARTICLES / 10);
			initialParticlePosition[i].pos = glm::vec2(2.0f * glm::fract(f) - 1.0f, 0.2f * glm::floor(f) - 1.0f);
			initialParticlePosition[i].vel = glm::vec2(0.0f);
		}

		dA.assign(initialParticlePosition);
		particlesBuffer_ = dA.mData.buffer->buffer;

		auto kernel = std::make_shared<CsyVk::VkProgram>(
			CsyVk::BUFFER(Vertex),
			CsyVk::CONSTANT(PushConstants));

		kernel->load((shadersPath + "particle.comp.spv"));
		...
		while (!ShouldQuit())
		{
			...
			if (!paused_)
			{
				...
				CsyVk::VkConstant<PushConstants> constants;
				constants.setValue(push_constants);

				kernel->enqueue(CsyVk::vkDispatchSize(NUM_PARTICLES, 128), dA.handle(), &constants);
				kernel->end();
