...
#define CONSTANT(T) constantPtr<T>()
	...
	VkProgram::VkProgram(Args... args)
	{
		...
		VkSemaphoreCreateInfo semaphoreCreateInfo = csyvk::initializers::semaphoreCreateInfo();
		...
		for (size_t i = 0; i < mFormalParamters.size(); i++)
		{
			...
			{
				setLayoutBindings.push_back(csyvk::initializers::descriptorSetLayoutBinding(VkVariable::descriptorType(mFormalParamters[i]->type()), VK_SHADER_STAGE_COMPUTE_BIT, i));
			}
		}
		...
		if (nBuffer > 0)
			poolSizes.push_back(csyvk::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, nBuffer));

		if (nUniform > 0)
			poolSizes.push_back(csyvk::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, nUniform));
		...
		for (size_t i = 0; i < variables.size(); i++)
		{
			...
			if ((mAllArgs[i]->type() == VariableType::DeviceBuffer && mAllArgs[i]->bufferSize() > 0) || mAllArgs[i]->type() == VariableType::Uniform) {
				writeDescriptorSets.push_back(
					csyvk::initializers::writeDescriptorSet(descriptorSet, VkVariable::descriptorType(variable->type()), i, &variable->getDescriptor()));
			}
		}
