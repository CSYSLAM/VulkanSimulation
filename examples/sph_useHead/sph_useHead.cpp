...
public:
	...
	void buildComputeCommandBuffer()
	{
		...
		if (graphics.queueFamilyIndex != compute.queueFamilyIndex)
		{
			VkBufferMemoryBarrier buffer_barrier = { VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT,
				0, compute.queueFamilyIndex, graphics.queueFamilyIndex, storageBuffer.buffer, 0, storageBuffer.size};
	...
	void prepareStorageBuffers()
	{
		...
		{
			Particle p;
			p.position = glm::vec2(-0.625f + PARTICLE_RADIUS * 2 * x, PARTICLE_RADIUS * 2 * y);
			p.velocity = glm::vec2(0.f);
			p.force = glm::vec2(0.f);
			p.density = 0.0f;
			p.pressure = 0.0f;
			x++;
			if (x >= 125)
	...
	void prepareGraphics()
	{
		...
		std::vector<VkVertexInputAttributeDescription> inputAttributes = {
			vks::initializers::vertexInputAttributeDescription(0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Particle, position))
		};
		...
		shaderStages[0] = loadShader(getShadersPath() + "sph_useHead/particle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "sph_useHead/particle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "sph_useHead/compute_density_pressure.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		VK_CHECK_RESULT(vkCreateComputePipelines(device, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipeline0));

		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "sph_useHead/compute_force.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
		VK_CHECK_RESULT(vkCreateComputePipelines(device, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipeline1));

		computePipelineCreateInfo.stage = loadShader(getShadersPath() + "sph_useHead/integrate.comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
